    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($fname) || empty($lname) || empty($email) || empty($password) || empty($birthnumber) || empty($Gender)) {
            ...
            if ($resultCheck > 0){
                ...
                $insert = "INSERT INTO Coach (fname, lname, email, password, salt, pNr, gender) VALUES ('".$FirstName."', '".$LastName."', '".$Email."', '".$hash."','".$unique_salt."', '".$birthnumber."', '".$Gender."');";
                $resultat = $connection->query($insert);

                $fname = $lname = $email = $password = "";

                header ("Location: T_registerCoach.php");
            }  
        }
    }
?>